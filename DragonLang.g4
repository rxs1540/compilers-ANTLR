...
// decls → decls decl | ε   (implemented as 0+ decl followed by epsilon print)
decls
...
type_rule returns [Type val]
    :   b=basic
        ...
        ( LBRACKET NUM RBRACKET
            {
                if ($val != null) $val.addDimension(Integer.parseInt($NUM.text));
            }
        )*
    ;
...
loc
    :   ID
        ...
        { System.out.println("loc -> id (lexeme = " + $ID.text + ")"); }
    ;
...
factor
    ...
    |   NUM
        {
            System.out.println("factor -> num (lexeme = " + $NUM.text + ")");
        }
    |   REAL
        {
            System.out.println("factor -> real (lexeme = " + $REAL.text + ")");
        }
    |   TRUE
